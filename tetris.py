...
class Shape():
    ...
    # Check if the rotated shape fits within the grid
        if self.x + len(rotated_shape[0]) <= 12 and self.y + len(rotated_shape) <= 24:
        # Check if there are any collisions with other blocks in the grid
            collision = False
            for y in range(len(rotated_shape)):
                for x in range(len(rotated_shape[0])):
                    if rotated_shape[y][x] == 1 and grid[self.y + y][self.x + x] != 0:
                        collision = True
                        break
                if collision:
                    break
        
        # If there are no collisions, update the shape
            if not collision:
                self.shape = rotated_shape
                self.height = len(self.shape)
                self.width = len(self.shape[0])
    
    # Redraw the shape on the grid
